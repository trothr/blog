...
Shell Oil in New Orleans, the summer that I met VM/SP.
...
(Easy to do with RPM. Dunno about other package registries.)
...
Chicory does not sign the packages, but the Chicory collection includes
...
In embracing the title, I have occasionally made available "gifts"
...
There was a change as of OpenSSH 9.0 which enables (now) reliable
file transfer (using 'scp', part of the OpenSSH package) in either
binary or text mode. Previously, it was difficult or impossible
I made a point to build OpenSSH 9.0 as broadly as possible.

This new capability is huge for those using `scp` to copy files
to/from USS, but we won't discuss the details in this post.

The latest release of OpenSSH is 9.1.
