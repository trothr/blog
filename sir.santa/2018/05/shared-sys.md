...
A long, long time ago (but I can still remember), a bunch of us
had this idea for shared operating systems among virtual machines.
...
Prior to 1998, there was no other virtualization than VM/ESA
...
the first to publicly port Linux to S/370. A talented team at IBM was
doing the same. Vepstas' work drove IBM to make their work public
...
"You can share the whole operating system image across dozens, hundreds,
...
and arranged the needed hacks to cope with the read-only thing.
...
was in having set-in-stone consistency, absolute defense against changes
...
As time went on, VMware became increasingly robust. It also caught the
...
(Barely noticable with z/VM, but significant with the others.)
...
terrific for testing, even a boon to security. Everything in the `chroot`
padded cell is isolated from the parent environment. (Though it uses
the same kernel and has the same network presence.)

FreeBSD took things up a notch. Around Y2K,
the FreeBSD people gave us the "jail" concept and others followed.
...
things in the parent that would be visible to an ordinary `chroot`.
...
Only that shared operating system hierarchies are an under-valued concept.
...
Mike asked, "Can we somehow lead?". We *have* to!
